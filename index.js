...
client.once('ready', async () => {
    const commands = [
        ...
        new SlashCommandBuilder().setName('mesai').setDescription('Mesai yönetimi')
            .addSubcommand(sub => sub.setName('kontrol').addUserOption(o => o.setName('kisi').setDescription('Kişiyi seç').setRequired(true)))
            .addSubcommand(sub => sub.setName('sıfırla').addUserOption(o => o.setName('kisi').setDescription('Kişiyi seç').setRequired(true)))
            .addSubcommand(sub => sub.setName('ekle').addUserOption(o => o.setName('kisi').setDescription('Kişiyi seç').setRequired(true)).addIntegerOption(o => o.setName('dakika').setDescription('Dakika gir').setRequired(true)))
    ];
    
    const rest = new REST({ version: '10' }).setToken(process.env.TOKEN);
    await rest.put(Routes.applicationCommands(client.user.id), { body: commands });
    console.log('✅ BCSO Botu tüm sistemleriyle aktif!');
});
    ...
    try {
        if (interaction.isChatInputCommand()) {
            if (interaction.commandName === 'kur-mesai') {
                const embed = new EmbedBuilder().setTitle('⚖️ BCSO MESAİ').setImage('https://media.discordapp.net/attachments/1498313566015717446/1498797722365460683/image.png').setDescription('Mesaide değilken botu açık bırakmanız mesainizin sıfırlanması ve strike 1 yemenizle sonuçlanır.').setColor('#2b2d31');
                ...
                if (sub === 'ekle') { mesailer.set(u.id, (mesailer.get(u.id) || 0) + interaction.options.getInteger('dakika')); await interaction.reply({ content: '✅ Eklendi.', ephemeral: true }); }
                else if (sub === 'sıfırla') { mesailer.set(u.id, 0); await interaction.reply({ content: '🧹 Sıfırlandı.', ephemeral: true }); }
                else { await interaction.reply({ content: `${u.username} toplam mesai: ${mesailer.get(u.id) || 0} dk.`, ephemeral: true }); }
            }
        }
        ...
        if (interaction.isModalSubmit()) {
            ...
            if (kanal) {
                const embed = new EmbedBuilder().setTitle('Yeni Başvuru').setDescription(`Başvuran: ${interaction.user}\n\nİçerik: ${interaction.fields.getTextInputValue('q1')}`);
